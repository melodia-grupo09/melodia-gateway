...
Given(
  'a playlist with name {string} and description {string}',
...
When(
  'the user requests the list of playlists',
...
When(
  'the user requests the playlist with name {string} by its ID',
...
When(
  'the user requests a playlist with ID {string}',
  async function (this: TestWorld, playlistId: string) {
    this.response = await request(this.app.getHttpServer()).get(
...
When(
  'the user deletes the playlist with name {string} by its ID',
  async function (this: TestWorld, name: string) {
    const playlist = this.playlists.get(name);
    assert.ok(
      playlist?.id,
      `Could not find a created playlist named "${name}" to delete`,
    );
...
When(
  'the user tries to delete a playlist with ID {string}',
  async function (this: TestWorld, playlistId: string) {
    this.response = await request(this.app.getHttpServer()).delete(
      `/playlists/${playlistId}`,
    );
...
When(
  'the user adds by ID the song with name {string} to the playlist with name {string}',
  async function (this: TestWorld, songName: string, playlistName: string) {
    const playlist = this.playlists.get(playlistName);
    const song = this.songs.get(songName);

    assert.ok(
      playlist?.id,
      `Precondition failed: Playlist "${playlistName}" not found in test context.`,
    );
    assert.ok(
      song?.id,
      `Precondition failed: Song "${songName}" not found in test context.`,
    );
...
When(
  'the user adds the song with id {string} to the playlist with name {string}',
  async function (this: TestWorld, songId: string, playlistName: string) {
    const playlist = this.playlists.get(playlistName);
    assert.ok(
      playlist?.id,
      `Precondition failed: Playlist "${playlistName}" not found in test context.`,
...
When(
  'the user adds the song with name {string} to the playlist with ID {string}',
  async function (this: TestWorld, songName: string, playlistId: string) {
    const song = this.songs.get(songName);
    assert.ok(
      song?.id,
      `Precondition failed: Song "${songName}" not found in test context.`,
...
Then(
  ...
  function (this: TestWorld, expectedName: string) {
    ...
    assert.strictEqual(
      ...
      expectedName,
      'The playlist name in the response is incorrect',
    );
    assert.ok(
      createdPlaylist.id,
      'The playlist in the response does not have an ID',
    );
  },
);

Then(
  'the list of playlists contains the playlist {string}',
  function (this: TestWorld, playlistName: string) {
    ...
    assert.ok(
      playlistExists,
      `The playlist "${playlistName}" was not found in the list`,
    );
  },
);

Then(
  'the details of the received playlist are correct',
  function (this: TestWorld) {
    ...
    assert.deepStrictEqual(
      ...
      'The playlist details do not match the created one',
    );
  },
);

Then(
  'the playlist {string} no longer exists in the list of playlists',
  async function (this: TestWorld, playlistName: string) {
    const deletedPlaylist = this.playlists.get(playlistName);
    assert.ok(
      deletedPlaylist,
      `Could not find playlist "${playlistName}" in the test context to verify deletion`,
    );
    ...
    assert.strictEqual(
      playlistExists,
      false,
      `The deleted playlist "${playlistName}" was unexpectedly found in the list`,
    );
  },
);

Then(
  'the playlist {string} contains the song {string}',
  function (this: TestWorld, playlistName: string, songName: string) {
    if (!this.response) {
      throw new Error('Response is not defined');
    }
