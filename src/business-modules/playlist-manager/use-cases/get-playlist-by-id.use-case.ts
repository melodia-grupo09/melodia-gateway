...
export class GetPlaylistByIdUseCase {
  ...
  async execute(id: UUID): Promise<PlaylistDTO> {
    const playlist = await this.playlistRepository.findOne({ id }, { populate: ['songs', 'songs.song'] })
    if (playlist === null) throw new NotFoundException('Playlist not found', `Playlist with ID ${id} not found`);
