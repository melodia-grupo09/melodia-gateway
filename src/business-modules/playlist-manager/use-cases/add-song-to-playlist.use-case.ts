...
export class AddSongToPlaylistUseCase {
  ...
  async execute(playlistId: UUID, songId: UUID): Promise<PlaylistDTO> {
    ...
    if (song === null) {
      throw new NotFoundException('Song not found', `Song with ID ${songId} not found`);
    }
    const playlist = await this.playlistRepository.findOne({ id: playlistId }, { populate: ['songs', 'songs.song'] });

    if (!playlist) {
      throw new NotFoundException('Playlist not found', `Playlist with ID ${playlistId} not found`);
    }
